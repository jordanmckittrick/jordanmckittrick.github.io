...
Obviously the risk-free asset is a major lemon, as it bleeds 3% each period. While it is less obvious, the risky asset is also a major lemon, which can be seen using Monte Carlo simulations. 
...
\end{align}
$$
The Chernoff bound then implies that $\mathbb{P}(S_n \geq 1) \leq e^{-n D_{\text{KL}}}$, which decays exponentially in $n$. @fig-no-loss-probability plots the exact tail probability against this bound.[^1]
...
#| fig-cap: "The probability of not having lost money after $n$ coin flips, $\\mathbb{P}(S_n \\geq 1)$, together with its Chernoff upper bound $e^{-n D_{\text{KL}}}$. This is the same quantity the bottom panel of @fig-risky-wealth estimates by simulation, resolved far into the tail."
...
4. The excess growth rate is naturally measured in bits, as it is the difference between two Kullback-Leibler divergences. 
