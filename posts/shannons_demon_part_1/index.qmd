...
\end{align}
$$
which defines $W(w)$ (in nats). For the risky asset, we have $w^T = (0,1)$; hence,
$$
\begin{align}
...
```
The key point of this plot is that the concavity of the growth-rate function is what allows us to almost surely make an exponential amount of money in the long run. We may intuitively think that the performance of a constant rebalanced portfolio at some $f$ should be somewhere in between the performance of the risk-free asset ($f = 0$) and the risky asset ($f = 1$). However, this is simply not the case. While it is true that the expected _return_ is a linear function of $f$, the expected _growth rate_ $W = W(f)$ -- which is the quantity that actually drives long-run performance [@breiman1961] -- is a concave function of $f$. The values of $f$ that fall between the two black "X" marks in @fig-arithmetic-vs-geometric are those that yield a positive expected growth rate, and hence almost surely $S_n \rightarrow \infty$. [@breiman1961] defines gambles such as these as "favorable". 
...
&= \ln r + p_h \ln \left(\frac{m_h(f)}{q_h} \right) + p_t \ln \left(\frac{m_t(f)}{q_t} \right) \quad \text{nats}, \\
\end{align}
$$
or
$$
\begin{align}
W(f) &= \log_2(e) \Big( \ln r + D_{\text{KL}}\left(\tilde{p} \| \tilde{q}\right) - D_{\text{KL}}\left(\tilde{p}\| \tilde{m}_f\right) \Big) \quad \text{bits}. \\
\end{align}
$$



In other words, the excess growth rate, $W(f) - \ln r$, is a measure of information. It is the difference between the KL divergence from the risk-neutral distribution $\tilde{q}$ to the actual distribution $\tilde{p}$ and the KL divergence from the tilted risk-neutral distribution $\tilde{m}_f$ to the actual distribution $\tilde{p}$. The first term is independent of $f$ and determines the maximum possible excess growth rate, while the second term is minimized by choosing $f$ so that $\tilde{m}_f = \tilde{p}$.
