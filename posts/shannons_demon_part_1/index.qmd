...
#| fig-cap: "Simulated wealth paths for a portfolio held entirely in the risky asset. Top: wealth as a multiple of starting wealth, drawn on a logarithmic scale, with the median, the mean, and the middle 95% of paths. Bottom: the fraction of paths still at or above break-even."
...
To see this, we begin with the observation that since the sequence of random variables is i.i.d., the strong law of large numbers implies that the empirical growth rate, $W_n$, converges almost surely to the expected growth rate $W(w)$ (or simply "growth rate"):
$$
\begin{align}
...
\end{align}
$$
which defines $W(w)$. For the risky asset, we have $w^T = (0,1)$; hence,
$$
\begin{align}
...
Given that a wealth path tends to zero with probability one, the expected value path is a mirage. The expected value is dominated by a small number of rare events, which are not representative of the typical outcome. 
...
#| fig-cap: "The probability of not having lost money after $n$ coin flips, $\\mathbb{P}(S_n \\geq 1)$, together with its Chernoff upper bound $e^{-n D_{KL}}$. This is the same quantity the bottom panel of @fig-risky-wealth estimates by simulation, resolved far into the tail."
...
Earlier I invoked the strong law of large numbers to show that the risky asset is a major lemon: because its expected growth rate, $W(w),$ is negative whenever $\alpha < 1$, it will almost surely tend towards a 100% loss in the long run, and as I just showed, the probability of not losing money decays exponentially fast. Instead of letting our portfolio consist solely of the risky asset, however, we can instead choose $w$ to maximize $W(w) = \mathbb{E} \left[ \ln Y(w) \right].$ This maximum is unique because $W$ is a concave function of $w$. If we think of $W(w)$ as a function of $f$ (since $w = (1 - f, f)$), we can find the optimal fraction to allocate to the risky asset by taking the derivative with respect to $f$ and setting it equal to zero. Doing so, we find that the optimal fraction is given by 
$$
\begin{align}
f^* = r \frac{\Delta}{ab},
\end{align}
...
We see immediately that $f^*$ is positive (i.e. we invest at least some of our money into the risky asset) if and only if $\Delta > 0$. If $\Delta = 0$, then $f^* = 0$ and we invest all of our money into the riskless asset. If $\Delta < 0$, then $f^* < 0$ and we would need to short the risky asset in order to maximize our expected growth rate. In our case, we are restricting ourselves to long-only portfolios (i.e. $0 \leq f \leq 1$), so we would simply invest all of our money into the riskless asset. Since the growth rate diverges to negative infinity at $f = -r/a < 0$ and $f = r/b \geq 1$, we see that the optimal $f^*$ is always in the interval $(-r/a, r/b)$.
...
\end{align}
$$
which, when $r = 1$, reduces to the most general form of the Kelly criterion for a single, two-state risky asset. In that context, $\Delta$ is often referred to as the "edge". If we not only let $r = 1$ but also $\gamma_t = 0$ (implying $b=1$), then we recover the Kelly criterion for $a$-to-$1$ odds: $f^* = \frac{p_h a - (1 - p_h)}{a}$, or "edge over odds".

For our specific example, we have $f^* \approx 0.38$, as shown in @fig-arithmetic-vs-geometric. In general, however, we will need to solve problems of this type numerically. At this expected-growth-rate-maximizing allocation, the growth rate is $W(f^*) \approx 0.018$ bits (or about $0.012$ nats). This means that, on average, we can expect to double our money every $1 / 0.018 \approx 55$ periods -- not bad for a portfolio of lemons.
...
Let's now simulate the performance of a portfolio using this optimal weighting to see whether my theoretical claims hold water. Using the same random number generator as before, I have simulated the performance of a portfolio using the optimal CRP strategy, as seen below. A striking feature of this simulation is that after only roughly 100 periods two thirds of wealth paths are at or above break-even.

```{python}
#| echo: false
#| label: fig-optimal-crp-wealth
#| fig-cap: "Simulated wealth paths for a portfolio using the optimal CRP strategy. Top: wealth as a multiple of starting wealth, drawn on a logarithmic scale, with the median, the mean, and the middle 95% of paths. Bottom: the fraction of paths still at or above break-even."
...
```
The key point of this plot is that it is the concavity of the growth rate function that allows us to almost surely make an exponential amount of money in the long run. Intuitively, we think that the performance of a constant rebalanced portfolio at some $f$ should be somewhere in between the performance of the risk-free asset ($f = 0$) and the risky asset ($f = 1$). However, this is simply not the case. While it is true that the expected _return_ is a linear function of $f$, the expected _growth rate_ $W = W(f)$ -- which is the quantity that actually drives long-run performace -- is a concave function of $f$. If there is one take-home message from this post, this is it.
...
Since $W$ drives long-run performance, let's take a closer look at it. We have
$$
\begin{align}
...
&= \log r + p_h \ln \left(1 + \frac{\Delta}{b} \right) + (1 - p_h) \ln \left(1 - \frac{\Delta}{a} \right) \\
&= \log r + p_h \ln \left(\frac{p_h(a+b)}{b} \right) + (1 - p_h) \ln \left(\frac{(1-p_h)(a+b)}{a} \right) \\
&= \log r + p_h \ln \left(\frac{p_h}{q_h} \right) + p_t \ln \left(\frac{p_t}{q_t} \right) \\
&= \log r + D_{\text{KL}}\left(\tilde{p} \| \tilde{q}\right), \\
\end{align}
$$
where $q_h = b / (a + b)$ and $q_t = a / (a + b)$ are alternative probabilities of heads and tails, respectively, where I've let $p_t = 1 - p_h$ for notational convenience, and where $D_{\text{KL}}(\tilde{p} \| \tilde{q})$ is the Kullback-Leibler (KL) divergence from the model $\tilde{q} = (q_h, q_t)$ to the actual distribution $\tilde{p} = (p_h, p_t)$. Interestingly, $\tilde{q}$ turns out to be the risk-neutral probability distribution for this setup, i.e. the expected return of both assets under $\tilde{q}$ is exactly the riskless return $r$:
$$
\begin{align}
\mathbb{E}_{\tilde{q}}[Y(f)] &= q_h Y_h(f) + q_t Y_t(f) = r,
\end{align}
$$
which is independent of $f$, a property we will come back to in a moment. Since $D_{\text{KL}}\left(\tilde{p} \| \tilde{q}\right) \ge 0$ with equality if and only if $\tilde{p} = \tilde{q}$, we have an edge whenever the actual distribution of returns differs from the risk-neutral distribution. The portfolio's excess growth rate over the riskless asset is identically the excess number of bits needed to encode the actual distribution of returns $\tilde{p}$ using a code optimized for the risk-neutral distribution $\tilde{q}$. Importantly, this tells us that, with probability one, in the long-run, one will always be able to beat the risk-free asset by an exponential amount whenever the actual distribution of returns differs from the risk-neutral distribution. 

We have discovered a formula for $W(f^*)$. Let's now try to find a formula for $W(f)$ more generally, i.e. where we no longer require $f = f^*$. To do this, notice that there is another probability distribution hiding in plain sight: since $\mathbb{E}_{\tilde{q}}[Y(f)] = r$ for all $f$, if we simply divide by $r$, we see that we have derived a new probability distribution $\tilde{m}_f = \left(m_h(f), m_t(f) \right)$ that depends on $f$:
$$
\begin{align}
m_h(f) &= \frac{q_h Y_h(f)}{r}, \quad m_t(f) = \frac{q_t Y_t(f)}{r}.
\end{align}
$$
Observe that $\tilde{m}_f$ is just a tilted version of the risk-neutral distribution $\tilde{q}$ by the discounted portfolio gross returns $Y(f)$, and that it is indeed a probability distribution since $m_h(f) + m_t(f) = 1$, and both terms are non-negative. 

This implies that we can write $W(f)$ as
$$
\begin{align}
W(f) &= p_h \log Y_h(f) + p_t \log Y_t(f) \\
&= p_h \log \left(\frac{q_h Y_h(f)}{r} \frac{r}{q_h} \right) + p_t \log \left(\frac{q_t Y_t(f)}{r} \frac{r}{q_t} \right) \\
&= \log r + p_h \log \left(\frac{m_h(f)}{q_h} \right) + p_t \log \left(\frac{m_t(f)}{q_t} \right) \\
&= \log r + D_{\text{KL}}\left(\tilde{p} \| \tilde{q}\right) - D_{\text{KL}}\left(\tilde{p}\| \tilde{m}_f\right).
\end{align}
$$
In other words, the excess growth rate, $W(f) - \log r$, is literally a measure of information. It is the difference between the KL divergence from the risk-neutral distribution $\tilde{q}$ to the actual distribution $\tilde{p}$ and the KL divergence from the tilted risk-neutral distribution $\tilde{m}_f$ to the actual distribution $\tilde{p}$. The first term is independent of $f$ and determines the maximum possible excess growth rate, while the second term is minimized by choosing $f$ so that $\tilde{m}_f = \tilde{p}$.
...
Unlike Maxwell's demon, Shannon's demon is, in some sense, real. More scientifically, the key takeaways from this post are:

1. Long-run performance is determined by the expected growth rate, $W(f) = \mathbb{E}[\log Y(f)]$, _not_ the expected return, $\mathbb{E}[Y(f)]$.
...
4. The excess growth rate is _literally_ measured in bits, as it is the difference between two Kullback Leibler divergences. 
5. The excess growth rate is only ever positive when the actual distribution $\tilde{p}$ of returns differs from the risk-neutral distribution $\tilde{q}$.
6. The optimal growth rate is achieved when the tilted risk-neutral distribution $\tilde{m}_f$ matches the actual distribution $\tilde{p}$.
