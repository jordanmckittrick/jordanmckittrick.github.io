...
### The setup
In order to most clearly illustrate the concepts at play, I have necessarily chosen a very simple (and therefore unrealistic) example, in which there are only two assets to invest in: a "risk-free" asset with a negative interest rate and a risky asset modelled by a coin flip. Nonetheless, the results and machinery I develop here generalize to more realistic settings, though of course there are additional nuances and complications that arise in those cases, such as in estimating returns probabilities, accounting for transaction costs, and so on. In this post, however, I will ignore such complications in order to focus on the core ideas.
...
The _expected value_ path for the risky asset, however, tells a very different story. After 500 periods, the expected value path (not shown in @fig-risky-wealth) for the risky assets sits around $4 \cdot 10^{39}$, which is dozens of orders of magnitude better than both the median and the (empirical) mean. This is because of the small number of rare paths with enough heads to make a killing. In fact, using the independence of the coin flips, we see that the expected wealth after $n$ periods diverges to infinity exponentially fast:
$$
\begin{align}
...
\end{align}
$$
The Chernoff bound then implies that $\mathbb{P}(S_n \geq 1) \leq e^{-n D_{KL}}$, which decays exponentially in $n$. @fig-no-loss-probability plots the exact tail probability against this bound.[^1]

[^1]: Note that the KL divergence appearing here is not directly related to the KL divergence that appears in section @sec-the-connection-to-information-theory, though they are both measures of information.
...
```
The key point of this plot is that it is the concavity of the growth rate function that allows us to almost surely make an exponential amount of money in the long run. We may intuitively think that the performance of a constant rebalanced portfolio at some $f$ should be somewhere in between the performance of the risk-free asset ($f = 0$) and the risky asset ($f = 1$). However, this is simply not the case. While it is true that the expected _return_ is a linear function of $f$, the expected _growth rate_ $W = W(f)$ -- which is the quantity that actually drives long-run performance [@breiman1961] -- is a concave function of $f$. The values of $f$ that fall between the two black "X" marks in @fig-arithmetic-vs-geometric are those that yield a positive expected growth rate, and hence almost surely $S_n \rightarrow \infty$. [@breiman1961] defines gambles such as these as "favorable". 



### The connection to information theory {#sec-the-connection-to-information-theory}
...
&= \ln r + D_{\text{KL}}\left(\tilde{p} \| \tilde{q}\right), \\
\end{align}
$$
