...
```
The key point of this plot is that it is the concavity of the growth rate function that allows us to almost surely make an exponential amount of money in the long run. We may intuitively think that the performance of a constant rebalanced portfolio at some $f$ should be somewhere in between the performance of the risk-free asset ($f = 0$) and the risky asset ($f = 1$). However, this is simply not the case. While it is true that the expected _return_ is a linear function of $f$, the expected _growth rate_ $W = W(f)$ -- which is the quantity that actually drives long-run performace [@breiman1961] -- is a concave function of $f$. The values of $f$ that fall between the two black "X" marks in @fig-arithmetic-vs-geometric are those that yield a positive expected growth rate, and hence almost surely $S_n \rightarrow \infty$. [@breiman1961] defines gambles such as these as "favorable". 

### The connection to information theory

Since $W(f^*)$ is an upper bound on long-run performance, let's take a closer look at it. We have
$$
\begin{align}
...
We therefore derive the key result of this post:
$$
\begin{align}
...
4. The excess growth rate is literally measured in bits, as it is the difference between two Kullback Leibler divergences. 
...
I hope to continue exploring these and similar ideas in future posts.
