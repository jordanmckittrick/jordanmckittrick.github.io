---
title: "1: Shannon's Demon, Made Exact"
...
image: fallen-angel.jpg # shannons_demon_fiend.svg
...
![Alexandre Cabanel, _Fallen Angel_, 1847. Musée Fabre.](fallen-angel.jpg)
...
![Thomas Moran, _The Mirage_, 1879. Stark Museum of Art.](the-mirage-1879.jpeg){#fig-moran width="100%" fig-align="center" fig-alt="A luminous painting of the American West by Thomas Moran. A small party of riders on horseback fords a broad, glassy expanse of shallow water that mirrors the red sandstone cliffs and buttes rising behind them, all beneath a wide, cloudless blue sky."}
...
&= \sum_{j=\lceil cn \rceil}^{n} \binom{n}{j} p_h^j (1 - p_h)^{n-j}, \\ 
\end{align}
$$
```{python}
#| echo: false
...
#| fig-cap: "Simulated wealth paths for a portfolio using the optimal CRP strategy. Top: wealth as a multiple of starting wealth, drawn on a logarithmic scale, with the empirical median, the empirical mean, and the middle 95% of paths (300 paths shown). Bottom: the fraction of paths still above break-even."
...
<!--
  ...
  color, each path's own SVG for the faint one), so nothing here has to know
